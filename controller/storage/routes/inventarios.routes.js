...
inventarios.post('/', (req, res) => {
    ...
    connection.query('SELECT * FROM inventarios WHERE id_producto = ? AND id_bodega = ?', [id_producto, id_bodega], (error, results) => {
        if (error) {
        ...
        if (results.length === 0) {
            ...
            connection.query('INSERT INTO inventarios (id_producto, id_bodega, cantidad) VALUES (?, ?, ?)', [id_producto, id_bodega, cantidad], (error) => {
                if (error) {
            ...
            connection.query('UPDATE inventarios SET cantidad = ? WHERE id_producto = ? AND id_bodega = ?', [newCantidad, id_producto, id_bodega], (error) => {
                if (error) {
